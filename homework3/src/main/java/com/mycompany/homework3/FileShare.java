...
public class FileShare {
  ...
  static void creaClaves(RSA rsa) throws Exception {
    /**
     * @author Alvaro leon
     * @see https://www.alvarodeleon.net/encriptar-y-desencriptar-con-rsa-en-java/ sections taken
     *     from the reference: genKeyPair(); saveToDiskPrivateKey();
     */
    rsa.genKeyPair(1024);
  ...
  public static void cifrarArchivo(RSA rsa) throws Exception {
    ...
    String rootProjectPath =
        rsa.getPublicKeyFile()
            .getAbsolutePath()
            .substring(
                0,
                rsa.getPublicKeyFile().getAbsolutePath().length() - RSA.PUBLIC_KEY_PATH.length());
    System.out.println(rootProjectPath);
    // Create a File Chooser for public key
    JFileChooser fcPublicKey =
        new JFileChooser(
            rsa.getPublicKeyFile()
                .getAbsolutePath()
                .substring(
                    0,
                    rsa.getPublicKeyFile().getAbsolutePath().length()
                        - RSA.PUBLIC_KEY_NAME.length()));
    ...
    JOptionPane.showMessageDialog(
        ...
        "Archivo encriptado guardado en el directorio:\n"
            + rootProjectPath
            + RSA.ENCRYPTED_FILES_FOLDER_NAME);
  }

  /**
   * Method that request a public key file, then a objective file and finally exports an decrypted
   * file on the decrypt folder
   *
   * @param rsa an instance of RSA class
   * @throws Exception Throws an exception when the main process does not finish as expected
   */
  public static void descifrarArchivo(RSA rsa) throws Exception {
    ...
    JFileChooser fcFileToDecrypt =
        new JFileChooser(rootProjectPath + RSA.ENCRYPTED_FILES_FOLDER_NAME);
  ...
  /**
   * Method that make a call to start the Cliente class with parameters: the public key path where
   * the file would be stored and the path where the encrypted file is stored
   *
   * @see Cliente
   */
  ...
  /**
   * Method that make a call to a instance of the class FileShare then use the method
   * descrifrarArchivo();
   *
   * @throws Exception Throws an exception when the main process does not finish as expected
   */
